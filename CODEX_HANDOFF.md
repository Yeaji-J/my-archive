...
Replies are stored compatibly in the existing `messages.body` column using the reserved `__ARCHIVE_REPLY__:` envelope. Read state must **not** be inserted into `messages`: doing so exposed internal `__ARCHIVE_READ__:` records as visible chat in older open clients. Read notifications now use Supabase Realtime broadcast events only, while the parser continues to suppress legacy receipt rows already created by the broken version. An outgoing message shows `1` until the other participant opens that room during the live session. Replies preserve the original message summary, show it above the new content, and can scroll back to a loaded original. Keep full chat and quick-chat parsing in sync whenever these envelopes change.
