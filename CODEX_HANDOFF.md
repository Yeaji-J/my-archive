...
Folders now support nested hierarchy through an optional `parentId` on each folder. Existing folders without `parentId` remain top-level without migration or data reset. The sidebar renders the hierarchy collapsed by default with expand/collapse controls and a per-folder child-add action; clicking a parent row toggles its children instead of leaving it permanently expanded. Sidebar folders can be drag-reordered among siblings at the same hierarchy depth, and explicit `order` values plus timestamps preserve that order through local and cloud saves. Opening a parent folder includes notes from all descendants; opening a child folder narrows to that branch. The writing-page toolbar exposes separate `상위 폴더` and conditional `하위 폴더` selectors, restoring both values from an existing note and saving the selected child folder through the normal note save path. Detail-view folder movement retains a compact hierarchical selector. Deleting a folder moves its direct notes and child folders one level upward instead of discarding them.

Folder names can be edited without recreating the folder: the sidebar exposes a hover pencil beside the delete button and the selected-folder context uses a directly editable, text-styled name field. Enter/blur commits through the normal folder save/sync path, Escape cancels, and folder identity, color, hierarchy, ordering, and contained notes remain unchanged.
...
- the adjacent `- 칼럼` action becomes available only after clicking inside a real multi-column row. The clicked column is tracked independently from the browser selection so moving to the toolbar cannot lose the delete target; deletion moves focus to the nearest surviving column, collapses a one-column remainder back to a normal full-width block, and persists through the normal memo auto-save path
...
- rendering a memo clears any accidental root-level alignment left behind by the browser editing engine; paragraph-level saved alignment remains authoritative. Invisible zero-width characters do not make an otherwise blank memo retain stale middle columns, and memo column rows explicitly flow left-to-right at full width
...
- memo album previews reuse the same canonical `720 × 680` full-page snapshot renderer as folder previews and scale the whole page to fit, instead of extracting only the first image or a few text fragments
...
- todo, wish, and shopping rows can be reordered by dragging the normally hidden three-dot handle or a non-interactive part of the row; text inputs keep their editing cursor and reordered arrays auto-save
- todo-list rows reveal the optional second-line memo automatically while the title/memo row is focused; selecting a linked note/folder immediately returns focus to that memo input so the user can continue typing without re-clicking. Empty helper text disappears again when focus leaves, while the linked target and saved memo content remain visible. Plain rows are vertically centered at a compact `47px` minimum height, while rows with saved memo content retain extra bottom breathing room. URLs and file locations remain supported, detected `http(s)`/`www` addresses remain safe clickable links in read-only views, and the editor still exposes a direct link action
...
- time tracker colors time blocks from `06` through `00`. Clicking a block already painted with the currently selected color clears it; clicking a differently colored block replaces it with the newly selected color immediately. Existing matching hours are migrated by hour rather than array position so changing the visible range does not shift saved records.
- each time-cell interaction resolves the current note data and matching slot by note/slot identity before painting. This keeps the selected COLOR chip, persisted block, and live totals on one canonical state even if cloud/local reconciliation replaced the data object after the editor first rendered.
...
- the time tracker has no row-level NOTE column. The right-side COLOR section provides a short saved project name per color (including custom colors), and those names appear in the duration summary. Project-name inputs are copied back from the live editor before any rerender or navigation and save on input/change/IME completion through both the archive save path and the time-tracker recovery snapshot.
- the time tracker alone shows a live color-by-color duration summary and a final total in a readable right-hand column beside the time grid; summary copy is at least `12px`, with no tinted background or left divider. Editor time rows use a `28px` minimum height to stay visually aligned with the right-side tools while remaining taller than compact preview rows. Every painted block counts as exactly 10 minutes. In the live editor the summary counts the exact rendered cell colors (and falls back to saved block colors outside the editor), preventing the visible grid and totals from drifting apart without rewriting user data
...
All archive list contexts—including folder, date, template overview, and each of the five template-specific lists—support select / select-all / bulk delete. The shared selection bar supports moving all selected notes to one top-level or nested folder; the move updates `folderId` through localStorage, IndexedDB durable snapshots, and Supabase cloud sync before selection mode closes. It never consumes a separate row: folder views place it beside the preview/list switch, template-specific views place it beside search, and date/template-overview modes place it at the right edge of the top browse controls. Selection mode reveals both move and bulk-delete actions there. Folder preview mode uses five columns on desktop.
...
Mobile responsiveness now has a layout-only guardrail layer in `css/responsive-theme.css`: top-level horizontal overflow is clipped, desktop fixed widths are removed from the post-it editor, archive filters/toolbars wrap, the memo toolbar and tags reflow, moodboard tools become a vertical grid with a vertically scrollable page, folders become a compact grid, and calendar/chat headers compress for narrow screens. The viewport supports safe-area insets. These rules intentionally do not alter input handlers, editor payloads, save scheduling, snapshots, or cloud synchronization; keep responsive work isolated from persistence unless a data change is explicitly required.

Chat now has a user-selectable font size in addition to the font family. Per-font visual scale compensation keeps the bundled handwriting, serif, and display fonts closer in apparent size. Message rows and quick-chat rows use the same typography variables and compact spacing, and image captions follow the sender/receiver text edge.

Replies are stored compatibly in the existing `messages.body` column using the reserved `__ARCHIVE_REPLY__:` envelope. Read state must **not** be inserted into `messages`: doing so exposed internal `__ARCHIVE_READ__:` records as visible chat in older open clients. Read notifications now use Supabase Realtime broadcast events only, while the parser continues to suppress legacy receipt rows already created by the broken version. An outgoing message shows `1` until the other participant opens that room during the live session. A received peer read timestamp is cached per signed-in user in localStorage so reloading that browser does not incorrectly restore `1`; fully offline cross-device read persistence would still require a dedicated server-side receipt table. Replies preserve the original message summary, show it above the new content, and can scroll back to a loaded original. Keep full chat and quick-chat parsing in sync whenever these envelopes change.
