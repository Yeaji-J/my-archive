...
- the adjacent `- 칼럼` action becomes available only when the caret is inside a real multi-column row. It removes that selected column, moves focus to the nearest surviving column, collapses a one-column remainder back to a normal full-width block, and persists through the normal memo auto-save path
