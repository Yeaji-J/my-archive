...
Folder visuals share the same broad, softly rounded, slightly hand-drawn silhouette across the dashboard, archive folder grid, selected-folder context, and compact sidebar. Avoid angular polygon corners, strong gradients, and saturated plastic-icon color contrast. An unselected folder appears closed; selecting a folder that actually contains notes reveals a subtle white paper sheet between the muted rear panel and pale front panel. Empty selected folders remain closed so the visual state reflects real content.
