...
Folder visuals share the same compact, softly irregular silhouette across the dashboard, archive folder grid, selected-folder context, and compact sidebar. Their target proportion is close to `1.2:1`, like the supplied mint folder reference, rather than a long horizontal icon. Corners should have restrained radii and must not look pill-shaped or cushion-like. Avoid angular polygon corners, strong gradients, and saturated plastic-icon color contrast. An unselected folder appears closed; selecting a folder that actually contains notes reveals a subtle white paper sheet between the muted rear panel and pale front panel. Empty selected folders remain closed so the visual state reflects real content.
