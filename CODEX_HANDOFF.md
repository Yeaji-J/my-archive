...
Read receipts and replies are stored compatibly in the existing `messages.body` column using reserved `__ARCHIVE_READ__:` and `__ARCHIVE_REPLY__:` envelopes. Receipt records must never be rendered or used as room previews. An outgoing message shows `1` until the other participant opens that room; receipt messages update and remove the indicator through realtime. Replies preserve the original message summary, show it above the new content, and can scroll back to a loaded original. Keep full chat and quick-chat parsing in sync whenever these envelopes change.
