(() => {
  ...
  function renderCounts(){
    ...
    countStarred.textContent =
      state.notes.filter(note => note.starred).length;
  }
  ...
  function renderSidebarFolders(){
    ...
    state.folders.forEach(folder => {
      ...
      li.className =
        'folder-item' +
        (currentView === folder.id ? ' active' : '');

      li.innerHTML = `
        <span class="folder-dot"
          style="background:${folder.color}"></span>
  ...
  function currentBreadcrumb(){
    ...
    return folder ? folder.name : '전체 자료';
  }
  ...
  function getFilteredNotes(){
    ...
    if(currentView === 'starred'){
      ...
      notes = notes.filter(
        note => note.folderId === currentView
      );
    }
  ...
  function renderFolderGridView(){
    ...
    const showFolders =
      currentView === 'all' && !searchTerm.trim();

    folderGrid.style.display =
      showFolders && state.folders.length
        ? 'grid'
        : 'none';
    ...
    if(showFolders){
      state.folders.forEach(folder => {
        ...
        card.addEventListener('click', event => {
          if(event.target.closest('.folder-card-del')) return;
          setView(folder.id);
        });

        card.querySelector('.folder-card-del')
      ...
      card.innerHTML = `
        <div class="note-card-top">
          ...
          ${note.starred ? `
            <span class="note-card-star">
              <svg viewBox="0 0 24 24">
                <path d="M12 2.5l2.9 6.2 6.6.7-5 4.6
                  1.4 6.6L12 17.6 6.1 20.6l1.4-6.6
                  -5-4.6 6.6-.7z"/>
              </svg>
            </span>
          ` : ''}
        ...
        <div class="note-card-bottom">
          ${
            folder
              ? `
                <span class="note-card-folder-dot"
                  style="background:${folder.color}"></span>
  ...
  function createNote(){
    ...
    const folderId =
      !['all', 'starred', 'chat'].includes(currentView)
        ? currentView
        : state.folders[0]?.id || '';
    ...
    colorSwatches
      ...
      .forEach(swatch => {
        swatch.addEventListener('click', () => {
          ...
          pendingFolderColor = swatch.dataset.color;
        });
      });
  ...
  function avatarGradient(id){
    ...
    return `linear-gradient(145deg,${first},${second})`;
  }
  ...
  async function ensureChatProfile(){
    ...
    if(!currentProfile){
      ...
      $('#profileUsernameInput').value =
        `${base}_${currentUser.id.slice(0, 4)}`
          .slice(0, 20);
  ...
  async function loadChatRooms(){
    ...
    chatRooms = rooms
      ...
      .sort((first, second) =>
        new Date(
          second.latest?.created_at || 0
        )
        - new Date(
          first.latest?.created_at || 0
        )
      );
  ...
  async function openChatRoom(roomId){
    ...
    $('#chatHeaderAvatar').textContent =
      initials(room.profile.display_name);
  ...
  $('#folderCancelBtn').addEventListener(
    'click',
    closeFolderModal
  );
  ...
  $('#newChatCloseBtn').addEventListener(
    'click',
    closeNewChat
  );
  ...
  $('#chatMobileBack').addEventListener(
    'click',
    () => {
      chatConversation.parentElement
        .classList.remove(
  ...
  $('#newNoteBtnSide').addEventListener(
    'click',
    createNote
  );

  $('#newNoteBtnTop').addEventListener(
    'click',
    createNote
  );
